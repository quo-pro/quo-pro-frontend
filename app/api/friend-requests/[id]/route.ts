...
const getFriendRequest = async (
  ...
  try {
    ...
    const response = await axiosHttp.get(`${apiRoutes.friendRequests}/${id}`);
    return apiResponse({
...
const updateFriendRequest = async (
  ...
  try {
    ...
    const response = await axiosHttp.patch(`${apiRoutes.friendRequests}/${id}`, payload);
    return apiResponse({
      status: response.status,
      data: response.data,
    });
...
const GET = withServerAuth(getFriendRequest);
const PATCH = withServerAuth(updateFriendRequest);
