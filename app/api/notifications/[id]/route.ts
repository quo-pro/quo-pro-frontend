...
const getNotification = async (
  ...
  try {
    ...
    const response = await axiosHttp.get(`${apiRoutes.notifications}/${id}`);
    return apiResponse({
...
const updateNotification = async (
  ...
  try {
    ...
    const response = await axiosHttp.patch(`${apiRoutes.notifications}/${id}`, payload);
    return apiResponse({
      status: response.status,
      data: response.data,
    });
...
const GET = withServerAuth(getNotification);
const PATCH = withServerAuth(updateNotification);
